...
<body>
    <section class="container d-flex justify-content-center align-items-center">
        <div class="container p-5 bg-white rounded-lg shadow-lg">
            ...
            <form action="<?= base_url('admin/edit_lomba_form') ?>" class="py-3" method="post" enctype="multipart/form-data">
                <div class="form-group mb-3">
                    <label for="nama_lomba">Nama Lomba</label>
                    <input type="text" id="nama_lomba" name="nama_lomba" value="<?= $lomba['nama_lomba'] ?>" class="form-control" aria-label="nama_lomba" required>
                </div>

                <input type="hidden" value="<?= $lomba['lomba_id'] ?>" name="lomba_id" id="lomba_id" placeholder="" class="form-control">
            ...
            <div class="input-box">
                ...
                <div id="akademik-options">
                    <select class="custom-select" name="prodi_id[]" id="inputGroupSelect01">
                        <option value="">Tidak Memilih Prodi</option>
                        <?php foreach ($prodi as $row) : ?>
                            <option value="<?= $row['prodi_id'] ?>" <?= (isset($lomba['prodi_lomba'][0]) && $row['prodi_id'] == $lomba['prodi_lomba'][0]) ? 'selected' : '' ?>>
                                <?= $row['nama_prodi'] ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <select class="custom-select mt-3" name="prodi_id[]" id="inputGroupSelect01">
                        <option value="">Tidak Memilih Prodi</option>
                        <?php foreach ($prodi as $row) : ?>
                            <option value="<?= $row['prodi_id'] ?>" <?= (isset($lomba['prodi_lomba'][1]) && $row['prodi_id'] == $lomba['prodi_lomba'][1]) ? 'selected' : '' ?>>
                                <?= $row['nama_prodi'] ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <select class="custom-select mt-3 mb-10" name="prodi_id[]" id="inputGroupSelect01">
                        <option value="">Tidak Memilih Prodi</option>
                        <?php foreach ($prodi as $row) : ?>
                            <option value="<?= $row['prodi_id'] ?>" <?= (isset($lomba['prodi_lomba'][2]) && $row['prodi_id'] == $lomba['prodi_lomba'][2]) ? 'selected' : '' ?>>
                                <?= $row['nama_prodi'] ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="input-box mb-10 mt-3">
                    ...
                    <input type="date" name="tenggat_pendaftaran" value="<?= isset($lomba['tenggat_pendaftaran']) ? $lomba['tenggat_pendaftaran'] : '' ?>" id="tenggat_pendaftaran" class="form-control">
                </div>
                <div class="input-box mb-10">
                    <label>Tanggal Lomba Dimulai</label>
                    <input type="date" name="tanggal_mulai" value="<?= isset($lomba['tanggal_mulai']) ? $lomba['tanggal_mulai'] : '' ?>" id="tanggal_mulai" class="form-control">
                </div>
                <div class="input-box mb-10">
                    <label>Tanggal Lomba Berakhir</label>
                    <input type="date" name="tanggal_selesai" value="<?= isset($lomba['tanggal_selesai']) ? $lomba['tanggal_selesai'] : '' ?>" id="tanggal_selesai" class="form-control">
                </div>
                <div class="input-box mb-10">
                    <label>Penyelenggara Lomba</label>
                    <input type="text" name="penyelenggara_lomba" value="<?=$lomba['penyelenggara_lomba'] ?>" id="penyelenggara_lomba" placeholder="" class="form-control">
                </div>
                <div class="input-box mb-10">
                    <label>Keterangan Lomba</label>
                    <textarea name="keterangan_lomba" id="keterangan_lomba" cols="30" rows="10" class="form-control"><?= isset($lomba['keterangan_lomba']) ? $lomba['keterangan_lomba'] : '' ?></textarea>
                </div>
                <div class="form-group mb-10">
                    <label for="poster">Poster Lomba</label>
                    <input class="form-control" type="file" id="poster" name="poster" accept=".jpg,.jpeg,.png">
                    <?php if (isset($lomba['poster_lomba']) && !empty($lomba['poster_lomba'])): ?>
                        <p class="file_sebelumnya">File yang diunggah sebelumnya: <a href="<?= $lomba['poster_lomba'] ?>" target="_blank"><?= $lomba['poster_lomba'] ?></a></p>
                    <?php endif; ?>
                </div>
