...
<body class="container d-flex justify-content-center align-items-center">
    <div class="container p-5 bg-white rounded-lg shadow-lg">
        ...
        <form action="<?= base_url('admin/mahasiswa/edit') ?>" class="py-3" method="post" enctype="multipart/form-data">
            <div class="form-group mb-3">
                <label for="NIM">NIM</label>
                <input type="text" id="NIM" name="NIM" class="form-control" placeholder="NIM" aria-label="NIM" value="<?= $mahasiswa['NIM'] ?>">
            </div>
            <div class="form-group mb-3">
                <label for="nama_lengkap">Nama Lengkap</label>
                <input type="text" name="nama_lengkap" class="form-control" placeholder="Nama Lengkap Mahasiswa" aria-label="nama_lengkap" value="<?= $mahasiswa['nama_lengkap'] ?>" value="<?= $mahasiswa['nama_lengkap'] ?>">
            </div>   
            <div class="form-group">
                <label for="email">Email address</label>
                <input type="email" name="email" class="form-control" id="email" aria-describedby="email" placeholder="Masukkan email mahasiswa" value="<?= $mahasiswa['email'] ?>">
            </div>
            <div class="form-group">
                <label for="password">Password</label>
                <input type="password" name="password" class="form-control" id="password" placeholder="Password" value="<?= $mahasiswa['password'] ?>">
            </div>
            <div class="input-group mb-3">
                ...
                <select class="custom-select" name="prodi_id" id="inputGroupSelect01">
                    ...
                    <?php foreach ($prodi as $row) : ?>
                        <option value="<?= $row['prodi_id'] ?>" <?= ($row['prodi_id'] == $mahasiswa['prodi_id']) ? 'selected' : '' ?>><?= $row['nama_prodi'] ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group">
                <label>IPK</label>
                <input class="form-control input-lg text-lg" name="ipk" min="0" max="4" step="0.01" type="number" value="<?= $mahasiswa['ipk'] ?>">
            </div>
            <div class="input-group mb-3">
                <div class="input-group-prepend">
                ...
                <div class="custom-file">
                    ...
                    <label class="custom-file-label" for="inputGroupFile01"><?php echo $mahasiswa['photo']; ?></label>
                </div>
            </div>
