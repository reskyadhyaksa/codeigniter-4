...
<head>
    ...
    <link rel="stylesheet" href="<?= base_url('css/detail_item.css') ?>" />
...
<body>
            ...
            <div class="row">                
                <div class="col-lg-8 mb-5 mb-lg-0">
                    <div class="blog_left_sidebar">
                        <div class="detail-foto-lomba">
                            <img src="<?= base_url('uploads/poster/' . $lomba['poster_lomba']) ?>" class="detail_poster">
                        </div>
                        <hr class="line"/>
                        <h3 class="judul-berita"><?= esc($lomba['nama_lomba'])?></h3>
                        <div class="container-tanggal-lomba">
                            <h5>Lomba dimulai pada </h5>
                            <h5 class="tanggal-mulai">
                                <?php
                                    $day = $days[date('l', strtotime($lomba['tanggal_mulai']))];
                                    $date = date('d', strtotime($lomba['tanggal_mulai']));
                                    $month = $months[date('F', strtotime($lomba['tanggal_mulai']))];
                                    $year = date('Y', strtotime($lomba['tanggal_mulai']));
                                    
                                    echo "$day, $date $month $year";
                                    ?>
                            </h5>
                            <h5>-</h5>
                            <h5 class="tanggal-selesai">
                                <?php
                                    $day = $days[date('l', strtotime($lomba['tanggal_selesai']))];
                                    $date = date('d', strtotime($lomba['tanggal_selesai']));
                                    $month = $months[date('F', strtotime($lomba['tanggal_selesai']))];
                                    $year = date('Y', strtotime($lomba['tanggal_selesai']));
                                    
                                    echo "$day, $date $month $year";
                                    ?>
                            </h5>
                        </div>
                        <h5 class="tanggal-pendaftaran">
                            <?php
                                $day = $days[date('l', strtotime($lomba['tenggat_pendaftaran']))];
                                $date = date('d', strtotime($lomba['tenggat_pendaftaran']));
                                $month = $months[date('F', strtotime($lomba['tenggat_pendaftaran']))];
                                $year = date('Y', strtotime($lomba['tenggat_pendaftaran']));
                                
                                echo 'Pendaftaran dibuka hingga ' . "$day, $date $month $year";
                                ?>
                        </h5>
                        <h5 class="nama-prodi"><?= implode(', ', esc($lomba['nama_prodi']))?></h5>
                        <hr class="line"/>
                        <p class="isi-berita">
                            <?php foreach ($lomba['keterangan_lomba'] as $paragraph): ?>
                                <p><?= $paragraph ?></p>
                            <?php endforeach; ?>
                        </p>
                    </div>
                ...
                <div class="col-lg-4">
                    ...
                    <aside class="single_sidebar_widget search_widget">
                        <form action="<?= base_url(($lomba['kategori_lomba'] == 'akademik') ? 'home/kategori_akademik' : 'home/kategori_nonakademik') ?>" method="get">
                            ...
                            <div class="cat-list-container">
                                <form action="<?= base_url(($lomba['kategori_lomba'] == 'akademik') ? 'home/kategori_akademik' : 'home/kategori_nonakademik') ?>" method="get">
                                    <div class="form-group">
                                        ...
                                        <input type="radio" id="prodi" name="category" value="prodi" checked>
                                        <label for="prodi">Akademik |</label>
                                        <input type="radio" id="non" name="category" value="non">
                                        <label for="non">Non-Akademik</label>
                                    </div>

                                    <ul class="list cat-list" id="prodi-list">
                                        <?php foreach ($prodi as $p): ?>
                                            <li>
                                                <form action="<?= base_url('home/kategori_akademik') ?>" method="get">
                                    ...
                                    <ul class="list cat-list" id="non-list" style="display: none;">
                                        <?php foreach ($non as $n): ?>
                                            <li>
                                                <form action="<?= base_url('home/kategori_nonakademik') ?>" method="get">
                                                    <input type="hidden" name="non_id" value="<?= esc($n['non_id']); ?>">
                                                    <button type="submit" class="d-flex btn btn-link">
                                                        <p><?= esc($n['nama_non']); ?></p>
                                                    </button>
                                                </form>
                            ...
                            response.notifications.forEach(function(notif, index) {
                                ...
                                var notifHTML = `
                                    <div class="${containerClass}">
                                        ...
                                        <form action="<?= base_url('notifikasi/mark_read_detail') ?>" method="post" enctype="multipart/form-data">
    ...
    document.querySelectorAll('input[name="category"]').forEach((elem) => {
        elem.addEventListener('change', function(event) {
            if (event.target.value === 'prodi') {
                document.getElementById('prodi-list').style.display = 'block';
                document.getElementById('non-list').style.display = 'none';
            } else {
                document.getElementById('prodi-list').style.display = 'none';
                document.getElementById('non-list').style.display = 'block';
            }
        });
    });
</script>
